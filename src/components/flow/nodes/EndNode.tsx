...
export const EndNode = memo(({ id, data, selected }: NodeProps) => {
  ...
  const [label, setLabel] = useState(nodeData?.label || "End");
  const inputRef = useRef<HTMLInputElement>(null);
  const { setNodes } = useReactFlow();

  useEffect(() => {
    setLabel(nodeData?.label || "End");
  }, [nodeData?.label]);
  ...
  return (
    <div
      className={cn(
        "group relative min-w-[140px] px-6 py-3 bg-[#ffebee] border-2 rounded-lg shadow-sm transition-all",
        selected
          ? "border-[#c62828] ring-2 ring-[#c62828]/20"
          : "border-[#c62828]/60"
      )}
    ...
    >
      {isEditing ? (
        <Input
          ...
          className="h-7 text-base font-semibold text-center border-[#c62828] bg-background"
          onClick={(e) => e.stopPropagation()}
        />
      ) : (
        <span className="text-base font-semibold text-center text-[#c62828] break-words">
          {label}
        </span>
      )}
      ...
      <Handle
        ...
        id="top-target"
        className="!bg-[#c62828] !border-2 !border-background !w-3 !h-3"
