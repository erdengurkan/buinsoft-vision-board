...
interface ProjectInlineEditProps {
  project: Project;
  option: "assign" | "deadline" | "priority" | "hardness" | "benefit";
  onSave: (updates: Partial<Project>) => void;
  onClose: () => void;
  mousePosition?: { x: number; y: number } | null;
}

export const ProjectInlineEdit = ({ 
  project, 
  option, 
  onSave, 
  onClose,
  mousePosition 
}: ProjectInlineEditProps) => {
  const [selectedAssignee, setSelectedAssignee] = useState(project.assignee || "");
  const [selectedDeadline, setSelectedDeadline] = useState<Date | undefined>(
    project.deadline ? new Date(project.deadline) : undefined
  );
  const [selectedPriority, setSelectedPriority] = useState<Priority>(project.priority);
  const [selectedHardness, setSelectedHardness] = useState<number | undefined>(project.hardness);
  const [selectedBenefit, setSelectedBenefit] = useState<number | undefined>(project.benefit);
